// StreamFi Frontend - FINAL PRODUCTION VERSION
// Your hackathon-winning app! 🚀
...
async function handleLogin(index) {
    ...
    try {
        ...
        if (data.success) {
            ...
            balanceIntervals[employee.name] = setInterval(async () => {
                await updateBalance(index);
            }, 1000);
            
            showNotification(`✅ ${employee.name} logged in! Streaming started.`, 'success');
            renderEmployees();
        }
    } catch (error) {
...
async function updateBalance(index) {
    ...
    try {
        ...
        const data = await response.json();
        balances[index] = data.balance;
        
        // Update UI
        const card = document.querySelector(`[data-employee-index="${index}"]`);
        if (card) {
            const balanceElement = card.querySelector('.balance-value');
            const claimButton = card.querySelector('.claim-button');
            
            if (balanceElement) {
                balanceElement.textContent = `${balances[index].toFixed(2)} STRM`;
            }
            
            if (claimButton) {
                claimButton.innerHTML = `${svg1}${svg2}${svg3}Claim ${balances[index].toFixed(2)} STRM`;
            }
        }
    } catch (error) {
        console.error(`Error updating balance for ${employee.name}:`, error);
    }
}

// Claim tokens button handler
async function handleClaim(index) {
    const employee = employees[index];
    const amount = balances[index];
    
    if (amount < 1) {
        showNotification('⚠️ Minimum 1 STRM required to claim!', 'warning');
        return;
    }
    
    try {
        showNotification('⏳ Sending transaction to Algorand TestNet...', 'info');
        ...
        if (data.success) {
            ...
            showNotification(
                `✅ Claimed ${data.amount} STRM!<br>` +
                `<small style="opacity: 0.9;">Company Wallet → Employee Wallet</small><br>` +
                `<a href="${data.explorer_url}" target="_blank" style="color: #1a1a1a; text-decoration: underline; font-weight: 600;">View Transaction on Explorer</a>`,
                'success',
                10000
            );
            ...
            showNotification(`❌ Claim failed: ${data.error}`, 'error');
        }
    } catch (error) {
...
async function handleLogout(index) {
    ...
    try {
        ...
        // Stop balance update interval
        if (balanceIntervals[employee.name]) {
            clearInterval(balanceIntervals[employee.name]);
            delete balanceIntervals[employee.name];
        }
        
        showNotification(`✅ ${employee.name} logged out!`, 'success');
        renderEmployees();
    } catch (error) {
...
// Show notification with BEIGE THEME! 🎨
function showNotification(message, type = 'info', duration = 5000) {
    const notification = document.createElement('div');
    
    // Beautiful beige/warm theme colors
    let bgColor, textColor, borderColor;
    
    switch(type) {
        case 'success':
            bgColor = '#cbb595';  // Your beautiful beige!
            textColor = '#1a1a1a';
            borderColor = '#b39f7f';
            break;
        case 'error':
            bgColor = '#d4a574';  // Warm terracotta
            textColor = '#1a1a1a';
            borderColor = '#c49464';
            break;
        case 'warning':
            bgColor = '#e5d4b8';  // Light beige
            textColor = '#1a1a1a';
            borderColor = '#cbb595';
            break;
        case 'info':
            bgColor = '#d9c8ad';  // Soft beige
            textColor = '#1a1a1a';
            borderColor = '#cbb595';
            break;
        default:
...
// Render employee cards - CLAIM BUTTON ALWAYS VISIBLE!
function renderEmployees() {
    ...
    employees.forEach((emp, index) => {
        ...
        card.setAttribute('data-employee-index', index);
        
        const isLoggedIn = loggedIn[index];
        ...
        card.innerHTML = `
            ...
            <div class="card-info">
                <div class="info-row">
                    ...
                    <span class="info-value gradient-text">${emp.rate} STRM/second</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Balance:</span>
                    <span class="info-value gradient-text balance-value">${balance.toFixed(2)} STRM</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Status:</span>
                    <span class="status-badge ${isLoggedIn ? 'status-streaming' : 'status-offline'}">
                        ${isLoggedIn ? '🟢 Streaming' : '⚫ Offline'}
                    </span>
                </div>
            </div>
            ...
            <div class="card-actions">
                ${isLoggedIn ? `
                    <button onclick="handleLogout(${index})" class="btn-stitch">
                        ${svg1}${svg2}${svg3}Logout
                    </button>
                    <button onclick="handleClaim(${index})" class="btn-stitch claim-button">
                        ${svg1}${svg2}${svg3}Claim ${balance.toFixed(2)} STRM
                    </button>
                ` : `
                    <button onclick="handleLogin(${index})" class="btn-stitch">
                        ${svg1}${svg2}${svg3}Login
                    </button>
                    <button onclick="handleClaim(${index})" class="btn-secondary-disabled claim-button-disabled" disabled>
                        ${svg1}${svg2}${svg3}Claim Now
                    </button>
                `}
            </div>
        `;
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    fetch(`${API_URL}/health`)
        ...
        .then(data => {
            if (data.status === 'ok') {
                showNotification('✅ Connected to Algorand TestNet!<br><small>2-Wallet System Active</small>', 'success', 3000);
            }
        })
        .catch(error => {
            showNotification('❌ Backend not connected! Start server first.', 'error', 10000);
        });
});
